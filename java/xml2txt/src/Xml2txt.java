...
// Xml2Txt = High-Performance XML to Flat-File Converter
// ============================================================

/**
 * xml2txt - Convert XML files to tab-separated text using xmlBluePrint library
 *
 * Usage: java xml2txt -p <blueprint.bp> -s <source_folder> -d <dest_folder> -t <threads> -n <pipeLineDeepN> -b <outputBufferSize>
 *
 * Phases:
...
public class Xml2txt {
    ...
    // Pipeline config
    private int C_PipeLineDeep = 4;
    
    private int C_bufferSize = 4;
    ...
    private void parseArgs(String[] args) {
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                case "-b":
                    if (i + 1 < args.length) C_bufferSize = Integer.parseInt(args[++i]);
                    break;
                case "-n":
                    if (i + 1 < args.length) C_PipeLineDeep = Integer.parseInt(args[++i]);
                    break;
                case "-p":
    ...
    private void mainLoop() throws Exception {
        ...
        System.out.print("Phase 2b: Processing files...\n...");
        ...
        System.out.println("\b\b\bPhase 2c: Draining pipeline...");
        ...
        if (xmlToken.msgError != null) {
            ...
            for (java.util.Map.Entry<Long, String> entry : xmlToken.buffOP.entrySet()) {
                ...
                if (writer != null) {
                    ...
                    // Batch disk flush: only every C_bufferSize xml, plus final at close
                    flushBatchCount++;
                    if (flushBatchCount % 2500 == 0) {
                        System.err.print('\b'+inProgress[idProgress]);
                        idProgress = (idProgress+1)%inProgress.length;
                    }
                    if (flushBatchCount % C_bufferSize == 0) {
                        writer.flush();
                    }
// bar removed
                }
